...
- 默认只读模式只显示 Sunshine 管理员明确授权的 mapping。
- 只有主机显式设置 `file_mapping_mode = full_disk` 后，配套 Sunshine 才会向已配对客户端列出进程能够访问的全部磁盘并允许写操作。
...
The default mode shows only explicitly authorized, read-only mappings. The companion BuffPlum Foundation Sunshine host exposes every readable drive and permits writes only after `file_mapping_mode = full_disk` is explicitly enabled. Use that mode only between personal devices on a trusted LAN, never expose its ports directly to the public Internet, run Sunshine with the least privileges needed, and keep independent backups. GitHub Actions packages are currently unsigned; download them only from the BuffPlum release page and verify the published checksums.
